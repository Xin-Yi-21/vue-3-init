...
// # 1、获取echart数据
const apiData = ref({})
const echartInfo = ref({ id: 'bar-echart', exportFileName: '柱状' })
...
function handleEchartInfo() {
  ...
  // 创建系列头
  let factor = [{ name: '降水', fieldT: 'rain', fieldN: 'rain', unit: 'mm', type: 'bar', yAxisIndex: 0 },]
  ...
  // 配置系列体
  let seriesOption = proxy.$getBarEchartOption({ echartInfo, settingStore, getType: 'include', optionList: ['series'] },)?.series[0] || {}
  chart.sData.forEach((item, index) => {
    let color = chart.color[index]
    let addOption = {
      itemStyle: { borderColor: color, color: color },
    }
    chart.sData[index] = proxy.$merge({}, seriesOption, addOption, item)
  })
  // 全局赋值
  echartInfo.value = Object.assign({}, echartInfo.value, chart)
  // console.log('柱状图', echartInfo.value)
  nextTick(() => { initEchart() })
}
// ^
// # 3、渲染echart
function initEchart() {
  let barOption = proxy.$getBarEchartOption({ settingStore, echartInfo, getType: 'exclude', optionList: ['series'] }) || {}
  let addOption = {
    title: { text: '柱状图' },
  ...
  let option = proxy.$merge({}, barOption, addOption)
